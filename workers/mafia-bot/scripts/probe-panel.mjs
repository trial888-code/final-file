...
const url = process.env.MAFIA_ADMIN_URL ?? "https://agentserver.mafia77777.com/admin/login";
const base = url.replace(/\/login.*$/i, "");
const cdpUrl = process.env.MAFIA_CDP_URL?.trim() || "http://127.0.0.1:9228";
...
const page =
  pages.find((p) => p.url().includes("mafia77777.com")) ?? pages[0] ?? (await context.newPage());
