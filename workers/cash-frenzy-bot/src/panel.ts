...
/**
 * Cash Frenzy layui admin (agentserver.cashfrenzy777.com/admin).
...
function isAdminDashboard(url: string): boolean {
  return (
    ...
    (url.includes("cashfrenzy777.com/admin") && !url.includes("/player/") && !url.includes("/login"))
  );
}
...
async function assertPageOpen(page: Page): Promise<void> {
  if (page.isClosed()) {
    throw new Error(
      "Cash Frenzy Chrome tab was closed. Keep /admin open in bot Chrome (port 9229) and do not close that window."
    );
  }
}
...
async function getListScope(page: Page): Promise<ListScope> {
  ...
  throw new Error(
    "Could not open User Management. In bot Chrome stay on https://agentserver.cashfrenzy777.com/admin " +
...
/** Cash Frenzy requires at least $1 initial recharge on the Add user form or creation fails. */
