...
const user = process.env.CASHFRENZY_AGENT_USERNAME;
const pass = process.env.CASHFRENZY_AGENT_PASSWORD;
if (!user || !pass) throw new Error("Missing CASHFRENZY_AGENT_USERNAME / CASHFRENZY_AGENT_PASSWORD in .env");

const cdpUrl = process.env.CASHFRENZY_CDP_URL?.trim();
...
if (cdpUrl) {
  ...
  page =
    pages.find((p) => p.url().includes("cashfrenzy777.com")) ??
    pages[0] ??
